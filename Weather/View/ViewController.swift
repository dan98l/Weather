...
class ViewController: UIViewController {
        ...
        DispatchQueue.main.async {
            if ((self.location?.latitude) != nil) {
                self.weather.fetchWeatherDataLocation(latitude: self.location!.latitude, longitude: self.location!.longitude) { city in
                     self.coreData.saveCityLocation(name: (city.last?.city!.name)!, temp: (Int((city.last?.list?.first?.main.temp!)!) - 273), latitude: (city.last?.city?.coord.lat)!, longitude: (city.last?.city?.coord.lon)!)
    ...
    @objc func timer() {
        Timer.scheduledTimer(timeInterval: 20, target: self, selector: #selector(self.timer), userInfo: nil, repeats: true)
        for i in self.allCity {
            self.weather.fetchWeatherDataLocation(latitude: i.latitude, longitude: i.longitude) { city in
                self.coreData.updateTempInCity(newTemp: (Int((city.last?.list?.first?.main.temp!)!) - 273), name: (city.last?.city!.name)!)
            }
        }
        ...
        self.weather.fetchWeatherDataLocation(latitude: lastLocation.coordinate.latitude, longitude: lastLocation.coordinate.longitude) { city in
            self.coreData.getCityLocation() { detCity in
                if detCity.count == 0 {
                    ...
                    self.tempMyCityLocation.text = "\(Int((city.last?.list?.first?.main.temp!)!) - 273) °С"
                    
                    self.coreData.saveCityLocation(name: (city.last?.city!.name)!, temp: (Int((city.last?.list?.first?.main.temp!)!) - 273), latitude: (city.last?.city?.coord.lat)!, longitude: (city.last?.city?.coord.lon)!)
                    self.coreData.saveMyCity(name: (city.last?.city!.name)!, temp: (Int((city.last?.list?.first?.main.temp!)!) - 273), latitude: (city.last?.city?.coord.lat)!, longitude: (city.last?.city?.coord.lon)!)
                    self.coreData.getMyCity() { city in
